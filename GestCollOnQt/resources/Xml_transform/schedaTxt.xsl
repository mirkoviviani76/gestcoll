...
ZECCA:       <xsl:value-of select="cc:nome"/> (<xsl:value-of select="cc:segno"/>) </xsl:for-each>
ZECCHIERE:   <xsl:for-each select="cc:zecchieri/cc:zecchiere"><xsl:value-of select="cc:nome"/><xsl:text> </xsl:text><xsl:value-of select="cc:segno"/> <xsl:value-of select="ruolo"/>; </xsl:for-each>
...
<xsl:if test="$conta2 != 0">
                <xsl:for-each select="cc:datiArtistici/cc:dritto/cc:legenda"><xsl:value-of select="cc:testo"/> <xsl:value-of select="cc:scioglimento"/></xsl:for-each></xsl:if>
R/:          <xsl:value-of select="cc:datiArtistici/cc:rovescio/cc:descrizione"/>
        ...
        <xsl:if test="$conta3 != 0">
                <xsl:for-each select="cc:datiArtistici/cc:rovescio/cc:legenda"><xsl:value-of select="cc:testo"/> <xsl:value-of select="cc:scioglimento"/></xsl:for-each></xsl:if>
...
NOTE:        <xsl:for-each select="cc:note/cc:nota">
* <xsl:value-of select="." />
</xsl:for-each>
